...
const RoleView = () => {
  ...
  const [drawerOpen , setDrawerOpen] = useState({ isOpen : false , roleId : -1 , roleName : "" })
  ...
  const handleOpenRoleAuthority = async ( role : Role ) => {
    ...
    setDrawerOpen({ isOpen : true , roleId : id , roleName : role.roleName })
  }
  ...
  const onClose = () => {
    setDrawerOpen({ isOpen : false , roleId : -1 , roleName : "" })
  }
  const onSure = async () => {
    await updateRole({
      id : drawerOpen.roleId ,
      roleName : drawerOpen.roleName ,
